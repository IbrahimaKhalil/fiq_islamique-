...
from langchain.retrievers import EnsembleRetriever # Import corrigé
...
class AlAkhdariEngine:
    def __init__(self, model_name="llama-3.1-8b-instant"):
        ...
        if not self.api_key:
            raise ValueError("🚨 Clé API GROQ introuvable.")
    ...
    def setup_rag(self, data_path="livres/*.pdf"):
        ...
        if not pdf_files:
            st.error(f"⚠️ Aucun PDF trouvé dans {data_path}")
            st.stop()
        ...
        self.retriever = EnsembleRetriever(
            retrievers=[bm25, vector_ret], weights=[0.5, 0.5]
        )
        self._build_chain()
...
{context}"""
    ...
    def ask(self, question, history=[]):
        if not self.chain:
            raise ValueError("Appelez d'abord setup_rag().")
        return self.chain.invoke({"question": question, "history": history})

# ============================================================
# 🎨 INTERFACE STREAMLIT
# ============================================================
st.set_page_config(page_title="Al-Akhdari AI", page_icon="🌙")
